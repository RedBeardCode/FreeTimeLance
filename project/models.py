...
def after_costumer_saved(sender, instance, created, *args, **kwargs):
    """
    Post saved function for the Costumer model, to create for each costumer a user group
    """
    if created:
        _ = Group.objects.get_or_create(name=instance.get_group_name())
...
def after_project_saved(sender, instance, created, *args, **kwargs):
    """
    Post save function to save the costumer user group as foreign key in the project.
...
class Project(models.Model):
    ...
    def get_durations_dump(self):
        """
        Creates a json list for the nv3d chart with the activities of the project
        """
        times = list()
        sum_time = timedelta()
